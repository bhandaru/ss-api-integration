...
def get_custom_field_value(field, val_type, x):
...
def send_to_smartsheet(d):
    #for i in range(1):
        print("Processing project#"+ ": " + d['name'])
        if d['name']== None or len(d['name']) == 0:
             print("Project name missing. Skiping #"+ str(n) + "..." )
        else:
            taskSheetId = copyProjectsTempalte(d['name'][:50])

            rowData =[]
            rowData.append({"Milestone":"Co-Creation"})
            rowData.append({"Description":d['name']})
            rowData.append({"Start Date":d['start_on']})
            rowData.append({"End Date":d['due_on']})
            rowData.append({"Priority":get_custom_field_value("Priority", "e", d)})
            rowData.append({"Assigned To":(d['assignee']['name'] if d['assignee'] != None else '')})

            parentRowId = writeRows(taskSheetId, rowData, None)
            row_num=2
            
            for j in range(len(d['subtasks'])):
                y=d['subtasks'][j]
...
for j in range(2):
    ...
    for i in range(len(data)):
        threads.append(threading.Thread(target=send_to_smartsheet, args=(data[i],)))
        threads[i].start()
...
def send_to_smartsheet(x):
    #for i in range(1):
        ...
        if x['name']== None or len(x['name']) == 0:
            ...
            rowData.append({"Priority":get_custom_field_value("Priority", "e", x)})
